...
void updateJoysticks(Joysticks* joysticks)
{
	...
	for (unsigned int playerIndex=0; playerIndex<Joysticks::maxXinputControllers; ++playerIndex)
	{
		...
		if (state->connected && XInputGetState(playerIndex, &xinput) == ERROR_SUCCESS) {
			...
			state->currentInputs[XboxInputLeftStickLeft]       = max(-xinput.Gamepad.sThumbLX / 32767.0f, 0);
			state->currentInputs[XboxInputLeftStickRight]      = max( xinput.Gamepad.sThumbLX / 32767.0f, 0);
			state->currentInputs[XboxInputLeftStickUp]         = max( xinput.Gamepad.sThumbLY / 32767.0f, 0);
			state->currentInputs[XboxInputLeftStickDown]       = max(-xinput.Gamepad.sThumbLY / 32767.0f, 0);
			state->currentInputs[XboxInputRightStickLeft]      = max(-xinput.Gamepad.sThumbRX / 32767.0f, 0);
			state->currentInputs[XboxInputRightStickRight]     = max( xinput.Gamepad.sThumbRX / 32767.0f, 0);
			state->currentInputs[XboxInputRightStickUp]        = max( xinput.Gamepad.sThumbRY / 32767.0f, 0);
			state->currentInputs[XboxInputRightStickDown]      = max(-xinput.Gamepad.sThumbRY / 32767.0f, 0);
